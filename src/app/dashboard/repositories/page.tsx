...
export default async function RepositoriesPage() {
  ...
  const [repos, integrations] = await Promise.all([
    fetchRepositories(workspace.workspaceId),
    getWorkspaceIntegrations(workspace.workspaceId),
  ]);

  // Check provider_connections table — not Supabase Auth identity
  const githubLinked = integrations.some(
      ...
      <PageHeader
        ...
        actions={
          githubLinked ? (
            ...
            <Link
              href="/api/integrations/github/connect?next=/dashboard/repositories"
